...
def example_use_of_gym_env():
    ...
    #goal reach action
    # MF=0, TL=1, TR=2, PK=3, UD=4
    #initial pos [2,1] dir [0,1]
    cost, done = step(env, MF) # pos [2,2] dir [0,1]
    cost, done = step(env, TR) # pos [2,2] dir [-1,0]
    cost, done = step(env, PK) # pos [2,2] dir [-1,0]
    cost, done = step(env, TR) # pos [2,2] dir [0,-1]
    cost, done = step(env, MF) # pos [2,1] dir [0,-1]
    cost, done = step(env, TR) # pos [2,1] dir [1,0]
    cost, done = step(env, MF) # pos [3,1] dir [1,0]
    cost, done = step(env, UD) # pos [3,1] dir [1,0]
    cost, done = step(env, MF) # pos [4,1] dir [1,0]
    cost, done = step(env, MF) # pos [5,1] dir [1,0]
